...
from ev2gym.utilities.loaders import (
...
class EV2Gym(gym.Env):
    def __init__(
        ...
        if load_from_replay_path is not None:
            ...
            if self.config["random_day"]:
                ...
                if self.config["simulation_days"] == "weekdays":
                    ...
                    self.config["simulation_days"] == "weekends"
                    and self.scenario != "workplace"
                ):
            ...
            self.replay = None
            self.sim_name = (
                f"sim_" + f"{datetime.datetime.now().strftime('%Y_%m_%d_%f')}"
            )
        ...
        self.sim_name = (
            extra_sim_name + self.sim_name
            if extra_sim_name is not None
            else self.sim_name
        )
    ...
    def reset(self, seed=None, options=None, **kwargs):
        ...
        self.early_termination = (
            False  # Flag for reward functions to signal early termination
        )
        self.unrecoverable_state = False  # Flag set when any EV has energy_gap > 0
        ...
        if self.simulate_grid:
            ...
            active_power, reactive_power = self.grid.reset(
                self.sim_date, load_data=load_data, pv_data=pv_data
            )

            self.node_active_power[1:, self.current_step] = active_power
            self.node_reactive_power[1:, self.current_step] = reactive_power
        else:
            self.node_active_power = np.zeros((34, self.simulation_length))
            self.node_reactive_power = np.zeros((34, self.simulation_length))
    ...
    def step(self, actions, visualize=False):
        ...
        if self.simulate_grid:
            for tr in self.transformers:
                self.node_ev_power[tr.id + 1 :, self.current_step] = tr.current_power
        ...
        # Update feasibility metrics for all connected EVs
        # This refreshes the cached attributes for the reward function and state
        for cs in self.charging_stations:
            for ev in cs.evs_connected:
                if ev is not None:
                    ev.update_feasibility_metrics(self.current_step)

        # Check for unrecoverable state: any EV with energy_gap > 0 cannot reach desired capacity
        self.unrecoverable_state = False
        for cs in self.charging_stations:
            for ev in cs.evs_connected:
                if ev is not None and ev.energy_gap is not None and ev.energy_gap > 0:
                    self.unrecoverable_state = True
                    self.early_termination = True
                    break
            if self.unrecoverable_state:
                break
    ...
    def _check_termination(self, reward, cost):
        ...
        # Check if a reward function has signaled early termination (e.g., infeasibility)
        truncated = getattr(self, "early_termination", False)
        if truncated:
            if self.verbose:
                print(
                    f"Episode truncated at step {self.current_step} due to early termination signal from reward function.\n"
                )
        ...
        for cs in self.charging_stations:
            ...
            for ev in self.departing_evs:
                if not self.lightweight_plots:
                    ...
                    self.port_current[ev.id, ev.location, self.current_step] = (
                        ev.actual_current
                    )
